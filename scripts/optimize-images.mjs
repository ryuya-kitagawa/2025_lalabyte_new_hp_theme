 ...
 * - PC版: distWebpDir + relDir + base.ext.webp（例: logobig.png.webp）
 * - SP版: distWebpDir + relDir + base-sp.ext.webp（例: logobig-sp.png.webp）
 */
function toWebpPathFromRelative(relPath, { isSp = false } = {}) {
    const relDir = dirname(relPath);
    const ext = extname(relPath);
    const base = basename(relPath, ext);
    const file = isSp ? `${base}-sp${ext}.webp` : `${base}${ext}.webp`;
